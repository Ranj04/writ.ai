...
Run directly (`python3 scripts/ci/test_writai_ci_check.py`) or through
...
import writai_ci_check as check  # noqa: E402,I001
...
class MarkerFileTests(unittest.TestCase):
    def test_task_file_binds_when_the_branch_does_not(self):
        with TemporaryDirectory() as directory:
            root = Path(directory)
            (root / ".writai").mkdir()
            (root / ".writai" / "task").write_text("TASK-102\n", encoding="utf-8")
            outcome = run(
    ...
    def test_a_symlinked_marker_is_refused(self):
        with TemporaryDirectory() as directory:
            ...
            os.symlink(str(target), str(root / ".writai" / "task"))
...
class GrantTests(unittest.TestCase):
    ...
    def test_allow_missing_grant_flag_turns_the_default_back_off(self):
        ...
        default = check.build_config(
            ...
            {"WRITAI_AGENT_URL": "http://agent.test"},
        )
...
class FailClosedTests(unittest.TestCase):
    ...
    def _main(self, opener_side_effect=None, payload=None, argv=None):
        environment = {"WRITAI_AGENT_URL": "http://agent.test"}
    ...
    def test_non_json_response_fails_the_check(self):
        ...
        with mock.patch.object(
            ...
            code = check.main(
                ...
                env={"WRITAI_AGENT_URL": "http://agent.test"},
            )
        self.assertEqual(code, check.EXIT_UNREACHABLE)
...
class UnresolvedAttachmentTests(unittest.TestCase):
    """`.writai/attach` must not be an opt-out from a required check.
    ...
    def _with_attach(self, contents: str, workspaces):
        with TemporaryDirectory() as directory:
            root = Path(directory)
            (root / ".writai").mkdir()
            (root / ".writai" / "attach").write_text(contents, encoding="utf-8")
    ...
    def test_it_fails_even_without_require_binding(self):
        ...
        with TemporaryDirectory() as directory:
            root = Path(directory)
            (root / ".writai").mkdir()
            (root / ".writai" / "attach").write_text("NOPE\n", encoding="utf-8")
    ...
    def test_an_unreadable_marker_still_falls_through(self):
        ...
        with TemporaryDirectory() as directory:
            root = Path(directory)
            (root / ".writai").mkdir()
            (root / ".writai" / "attach").write_text("A\nB\n", encoding="utf-8")
            outcome = run(
                [workspace()],
...
class MalformedResponseTests(unittest.TestCase):
    ...
    the `.writai/attach` marker settled: absence of binding information is
