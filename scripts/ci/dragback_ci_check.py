#!/usr/bin/env python3
"""Dragback pull-request authorization backstop.

The Claude Code ``PreToolUse`` hook fails **open**: a timeout, a crash, or an
unreachable supervisor lets the tool call proceed. ``hooks/dragback_hook_lib.py``
...
ENV_AGENT_URL = "DRAGBACK_AGENT_URL"
ENV_TIMEOUT = "DRAGBACK_CI_TIMEOUT_SECONDS"
ENV_API_KEY = "DRAGBACK_CI_API_KEY"
API_KEY_HEADER = "X-Dragback-Hook-API-Key"
...
class MalformedServiceResponse(ServiceUnreachable):
    ...
    `.dragback/attach` marker settled: *absence* of binding information is
...
def build_config(args: argparse.Namespace, env: Dict[str, str]) -> Config:
    ...
    if not agent_url:
        ...
        if is_github_actions(env):
            raise UsageError(
                "No Dragback agent URL configured. Set the DRAGBACK_AGENT_URL "
...
def fetch_workspaces(config: Config, opener: Any = None) -> List[Dict[str, Any]]:
    ...
    request = urllib.request.Request(
        ...
        headers={"Accept": "application/json", "User-Agent": "dragback-ci-check/1"},
        method="GET",
    )
    if config.api_key:
...
def resolve_binding(
    ...
    """Explicit attachment, then branch task ID, then ``.dragback/task``, then unbound.
    ...
    if attached is not None:
        # `dragback dev attach` writes the assignment ID alone, so the match is
        ...
        if len(matches) == 1:
            return Binding(
                SOURCE_EXPLICIT,
                "Bound by an explicit dragback attach selection.",
                matches[0],
            )
        ...
        # one junk `.dragback/attach` would be a way to opt a branch out of a
        # required check. `unresolved_explicit` makes `evaluate` fail it
        # regardless of --require-binding.
        return Binding(
            SOURCE_UNBOUND,
            ".dragback/attach does not name one available live assignment.",
    ...
    if task_id is not None:
        ...
        if len(task_matches) == 1:
            return Binding(
                SOURCE_TASK_FILE,
                "Bound by the repository .dragback/task file.",
                task_matches[0],
            )
        if len(task_matches) > 1:
            return Binding(
                SOURCE_UNBOUND,
                ".dragback/task matches assignments in multiple workspaces.",
            )
        return Binding(
            SOURCE_UNBOUND,
            ".dragback/task does not name an available live assignment.",
        )

    return Binding(
        SOURCE_UNBOUND,
        "No explicit attachment, branch task ID, or .dragback/task binding was "
...
def read_marker_file(repo_root: Path, name: str) -> Optional[str]:
    """``.dragback/<name>``: one non-empty line, small, a regular file.
...
def evaluate(
    ...
    if not binding.is_bound:
        ...
        # --require-binding says. Passing it would make `.dragback/attach` an
        ...
        if binding.unresolved_explicit:
            return Outcome(
                ...
                now_required=[
                    "Correct .dragback/attach to name a live assignment, or "
                    "delete it and let the branch name or .dragback/task bind."
                ],
                binding=binding,
            )
        if config.require_binding:
            return Outcome(
                ok=False,
                code="UNBOUND",
                headline="this branch resolves to no Dragback assignment",
                detail=binding.detail,
                exit_code=EXIT_UNAUTHORIZED,
                now_required=[
                    "Name the task in the branch, or run `dragback dev attach "
                    "<assignment-id>` and commit .dragback/task."
                ],
                binding=binding,
            )
        return Outcome(
            ok=True,
            code="UNBOUND",
            headline="no Dragback assignment governs this branch",
            detail=binding.detail,
            binding=binding,
            notes=[
    ...
    if candidate.state == STATE_INTERRUPTED:
        return Outcome(
            ...
            or "Dragback invalidated this task through the recorded provenance path.",
            exit_code=EXIT_UNAUTHORIZED,
            binding=binding,
            **narrative
        )
    ...
    if candidate.state == STATE_CONTINUING:
        return Outcome(
            ...
            detail=(
                "Dragback preserved this assignment; its snapshot is intentionally "
...
def build_narrative(
    ...
    if evidence_refs:
        ...
        evidence_ref = "dragback://workspaces/%s/assignments/%s" % (
...
def render(outcome: Outcome, config: Config) -> str:
    ...
    lines.append(INDENT + leader + "  DRAGBACK — " + outcome.headline)
    ...
    if outcome.provenance:
        ...
        lines.append(BODY + "".ljust(5) + "dragback dev why")
...
def write_step_summary(text: str, env: Dict[str, str]) -> None:
    ...
    try:
        with open(path, "a", encoding="utf-8") as handle:
            handle.write("## Dragback authorization\n\n```\n")
            handle.write(text)
            handle.write("\n```\n")
    except Exception:
        pass
...
def main(
    ...
    try:
        ...
        outcome = Outcome(
            ...
            exit_code=EXIT_UNREACHABLE,
            now_required=[
                "Restore the Dragback agent service, then re-run this check. "
