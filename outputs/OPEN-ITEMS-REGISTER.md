...
Defects and limits found during the build, with their current disposition. Nothing
here is a silent omission; each one is resolved, tracked work, or a deliberate
property.

| ID | Class | Current disposition |
|---|---|---|
| A1-1 | **Tracked design work** | A per-assignment pending-interrupt queue would preserve and render more than one outstanding correction. |
| A1-2 | **Tracked design work** | A workspace revision token plus UI re-confirmation would make preview-to-interrupt optimistic concurrency explicit. |
| A1-3 | **Deliberate property** | Preview remains a safe upper bound because the process-local session registry is not part of the interrupt-port contract. |
| A1-4 | **Resolved (superseded)** | SQLite is the default atomic workspace store; JSON remains an explicitly tested compatibility path. |
| A1-5 | **Tracked design work** | A durable shared event bus with replay and retention semantics would replace the process-local broker. |
| INT-2 | **Work to do** | Move the PR check to canonical workspace-context grant verification once its service URL, authentication, and plan transport are designed. |
| DEMO-ENV-1 | **npm advisories.** Recorded originally as 4 low, all via `elliptic` under `@hexclave/shared`. A later audit found 10 (4 low, 3 moderate, 3 high): the row had gone stale as new advisories were published against the existing tree. A plain `npm audit fix` cleared the moderates and one high; `pdfjs-dist` was then upgraded to `6.3.289` on its own, closing GHSA-hq66-cqwq-w95j - *arbitrary JavaScript execution upon opening a malicious PDF* - which this product reaches directly, because workspace document import accepts uploaded PDFs. | Low (4 remaining) | **RESOLVED except `elliptic`.** 4 low remain, all the same unpatched chain reached through the Hexclave SDK; no patched version exists, and that SDK is now lazily loaded so it is not in the eager bundle. The pdfjs major is flagged breaking by npm, so it was upgraded separately and verified by typecheck against the v6 API the extractor uses, the worker still resolving from `build/pdf.worker.min.mjs`, and a green build. **No test parses a PDF**, so that path is verified by construction rather than execution - worth knowing before trusting it blindly. |
| DEMO-ENV-2 | **Resolved** | The acknowledgement script now consumes every session-list column and is again the staged human-acknowledgement path. |
| DEMO-ENV-3 | **Resolved** | The stale editable install and missing `writai` entry point were repaired. |
| DEMO-ENV-4 | **Deliberate property** | The build-lane text remains historical and now points to current deny-until-acknowledged guidance. |

A resolved item keeps its original wording plus its resolution, because the record
...
|---|---|---|---|
| A1-1 | **Two decisions interrupting the same assignment overwrite each other's `interrupt_reason`, `redirect_instruction`, and `provenance_path`.** The newest interrupt wins and the earlier explanation is lost. | Medium | **TRACKED DESIGN WORK.** Correct behaviour takes a queue of pending interrupts per assignment and rendering/acknowledgement semantics for that queue. The demo path applies one decision. `applied_interrupts` does retain each decision's own partition, so the history is not lost — only the assignment's rendered explanation is. |
| A1-2 | **`preview()` and `interrupt()` are separate reads with no revision check.** State can change between the approver seeing "3 of 5 will be interrupted" and approving, so the applied blast radius can differ from the previewed one. | Medium | **TRACKED DESIGN WORK.** It takes an optimistic-concurrency token on the workspace record and a re-confirm path in Lane B's approval UI. |
| A1-3 | **The blast radius counts assignments with no bound Claude Code session.** A `RUNNING` assignment nobody is actually working on is counted as "will be interrupted" although no `PreToolUse` hook will ever fire for it. | Medium (was High) | **DELIBERATE PROPERTY.** `test_the_blast_radius_equals_the_set_that_actually_gets_denied` proves previewed == denied when all five sessions are bound. Otherwise the blast radius is an upper bound, never an under-count. Closing the gap is not a small intersection: `WorkspaceSupervisorInterruptPort` has a repository-only contract, while `ClaudeCodeSessionRegistry` is process-local at the agent-service composition root. It would take a shared/durable registry (or a new port dependency and explicit same-process limitation), composition changes, and preview tests for unbound and cross-process sessions. |
| A1-4 | **The JSON repository read-modify-write is not atomic across processes.** Two agent services, or a concurrent orchestrator save, can lose an interrupt. | Low (known) | **RESOLVED — superseded by the SQLite default.** `workspace_store` now defaults to `.writai/live-workspaces.sqlite3`; `test_concurrent_saves_from_two_processes_do_not_lose_an_update[sqlite]` proves its `BEGIN IMMEDIATE` write path. The opt-in JSON compatibility path stays honest through three strict xfails: cross-process atomicity, O(n) reparsing, and its world-readable write window. |
| A1-5 | **`services/events.py` `EventBroker` is process-local with a 100-event history.** | Low (known, pre-existing) | **TRACKED DESIGN WORK.** Replacing it takes a durable shared bus plus delivery, replay, retention, and multi-machine deployment semantics. |
...
| INT-2 | **The PR check's grant validation ignores `run_id`, `task_id` and `plan_hash`.** An ALLOW grant for another run, task or plan passes that weaker check when its snapshot and expiry match. | Medium | **WORK TO DO.** Canonical verification now exists at the workspace authority-context `/grants/verify` endpoint, and the executor uses it with all three bindings. Moving the stdlib 3.9 PR check to it is more than a small change: the check needs a separately configured authority URL, internal-service authentication, the full current plan, the workspace authority `context_id`, and fail-closed transport/result handling. Until that secure CI configuration and transport are designed, keep the local weaker check rather than copy the rules again. Executor enforcement remains canonical. |
...
| # | Item | Severity | Disposition |
|---|---|---|---|
| DEMO-ENV-1 | **4 low-severity npm advisories**, all one transitive edge: `elliptic` under `@hexclave/shared`, inherited by `@hexclave/react` and `@hexclave/ui`. `npm audit fix` cannot resolve them without changing the Hexclave dependency itself. | Low | **RESOLVED AS FAR AS A NON-BREAKING UPDATE ALLOWS.** Plain `npm audit fix` removed the fixable advisories. Five vulnerability entries remain: 4 low through the Hexclave SDK (`@hexclave/react` → `@hexclave/shared` / `@hexclave/ui` → `elliptic`), for which no patched `elliptic` version is available, and 1 high in `pdfjs-dist`. The SDK is lazily imported and is absent from the eager bundle. `pdfjs-dist` is load-bearing for document extraction and only has a semver-major `--force` fix, so it was deliberately not force-upgraded. |
| DEMO-ENV-2 | **`scripts/demo/ack.sh` renders a garbled blocked-session line** — `blocked by yesnonointerruptedgraph-v17graph-v18`, several fields concatenated with no separators — then reports `[FAIL]` for a session that `writai dev ack` considers not blocked. | Low | **RESOLVED.** The script read a 12-column unit-separator record into six variables, so Bash folded columns 6–12 into `decision_id`. It now consumes all 12 columns and passes only the service-reported decision id to `writai dev ack`. The script was kept because deny-until-human-acknowledgement is the current staged path and `STAGE_RUNBOOK.md` now explicitly invokes it. |
...
| DEMO-ENV-4 | **`docs/BUILD_LANE_A.md` still specifies "deny once"** (line ~99), which INT-3 superseded with deny-until-acknowledged. | Low | **DELIBERATE PROPERTY.** The historical wording remains intact. A dated note now points readers to current deny-until-acknowledged behaviour and `STAGE_RUNBOOK.md`. |
...
| # | Item | Disposition |
|---|---|---|
| T0-1 | **`uv.lock` is still consumed by nothing.** T0.1 capped `pyproject.toml` (`svix<2` and ceilings on every dependency) so a fresh `pip install` resolves the version that passes. Moving `scripts/bootstrap.sh` and CI to `uv sync --locked` is the durable fix. **Stale since `7da68d2`** ("Pin the actions, add the missing community files, drop uv.lock"): `uv.lock` was deleted there, and the commit message records why it was worse than unused (its `dev` extra lacked `pytest-cov`, which `pyproject.toml` makes mandatory, so `uv sync --locked` could not have run the suite). Verified absent on `close-final` with `ls uv.lock` and `git log --all -- uv.lock`. `CONTRIBUTING.md` records the dependency story: `pip` against the capped `pyproject.toml`, one gate script. | **RESOLVED** by `7da68d2`; this entry was stale. |
| T0-2 | **Two documented variables are read by nothing in the tree:** `WRITAI_PUBLIC_WEBHOOK_URL` and `HEXCLAVE_PUBLISHABLE_CLIENT_KEY` in `.env.example`. `test_every_documented_env_var_has_a_settings_field` allow-lists them literally with that note rather than deleting operator documentation. **Re-verified:** a `git grep` over the whole tree (`scripts/`, `frontend/`, `hooks/`, `.github/` included; `outputs/` and `.sol/` excluded) finds the two names only in `.env.example`, the allow-list and `docs/CONFIGURATION.md`. Nothing reads them, so they were dropped rather than wired: removed from `.env.example` (the Composio public-URL guidance stays as a comment, since it is still true and belongs to Composio's side), from `_READ_OUTSIDE_SETTINGS`, and from the `CONFIGURATION.md` exceptions table. The allow-list now accounts for every documented variable with a real reader. | **RESOLVED.** Dropped, not wired. |
...
| B1-3 | **Sibling stores inherit the `.sqlite3` suffix but stay JSON documents.** `services/authority_api.py:731` `_workspace_store_sibling` and `services/agent_api.py:680` `_crustdata_replay_store` derive `live-workspaces-<label>.sqlite3` names for `JsonCrustDataDeliveryReplayStore` and the Slack approval-thread / delivery stores. Functional; misleadingly named. T0-3 predicted this. Not changed in B1 — it is not the enforcement hot path and it is not a `LiveWorkspaceRepository`. **Closed:** `workspace_store_sibling(workspace_store, label)` in `workspaces/repository.py` derives `<stem>-<label>.json` next to the workspace store whatever its suffix; `authority_api._workspace_store_sibling` and `agent_api._crustdata_replay_store` both go through it, so the three ledgers are `live-workspaces-{slack-approval-threads,slack-deliveries,crustdata-deliveries}.json`. A JSON or suffix-less workspace store already produced that name, so those deployments see no change. **An operator's file under the old name is refused, not migrated:** if `<stem>-<label>.sqlite3` exists and the `.json` path does not, the helper raises naming both paths and saying the file was neither read nor moved. Refusal was chosen over the workspace repository's copy-on-first-start because these stores are opened lazily by per-request factories, so a "first start" copy would run mid-request and a failure halfway would leave a half-written replay ledger; refusing leaves the file exactly as found, the same stance as `_require_sqlite_database`. `scripts/demo/reset.sh` now derives the `.json` name and also clears the inherited-suffix name, so a reset cannot leave a file that trips the refusal. Tests: `backend/tests/test_workspace_store_siblings.py` (name is `.json` for `.sqlite3`/`.db`/`.json`/no suffix; the inherited file is refused with both paths in the message and left byte-identical; the refusal lifts once the `.json` path exists; both services derive through the helper). Control: with the suffix inheritance and the refusal hand-restored, four tests fail (`...sqlite3 == ...json` three times, `DID NOT RAISE RuntimeError` once); restored, all pass. | **RESOLVED.** Refuses on the old name; nothing migrated or renamed automatically. |
...
| B2-2 | **`HookApiKeyVerifier` survives as an alias of `HookCredentialVerifier`.** `backend/tests/test_five_session_demo.py:438-451` constructs `HookApiKeyVerifier(expected_api_key="test-key")` and is in no track's ownership row, so the rename keeps the old name and the `expected_api_key` constructor field (`supervisor_api.py`, the comment above the alias says why). Drop the alias when that caller moves. **Closed:** the alias is removed. The premise that one caller held it was short by two files: `test_claude_code_enforcement.py` (four sites) and `test_supervisor_runtime.py` (import plus the alias's own test) also constructed `HookApiKeyVerifier`; all three files now construct `HookCredentialVerifier`. **`expected_api_key` stays**, because removing it changes the single-key fallback path: `from_environment` reads `WRITAI_HOOK_API_KEY` into it (`supervisor_api.py:83`) and `_configured` maps it to developer `default` last, after the per-developer map, so a secret present in both resolves to `default`. Folding it into `credentials` would silently change that precedence and the "one secret for two developers" check. The field was never part of the alias. Test: `test_the_compatibility_alias_is_gone_and_the_single_key_field_stays` (`test_supervisor_runtime.py`) asserts the module has no `HookApiKeyVerifier` attribute and that `HookCredentialVerifier(expected_api_key=...)` still resolves to `default`; it replaces `test_the_single_key_verifier_name_still_constructs`, which existed only to keep the alias constructing. Control: with the alias hand-restored it fails `assert not True where True = hasattr(supervisor_api, 'HookApiKeyVerifier')`; restored, it passes. | **RESOLVED.** Alias removed; `expected_api_key` kept on purpose. |
...
| B2-4 | **`GET /supervisor/sessions` gains `owner_id` on every entry** via `RegisteredSession.to_payload`. `cli_dev.py:621-623` and `scripts/ci/writai_ci_check.py` accept extra keys, so nothing renders wrong; the list is still not filtered by owner (it is an authenticated read model for `dev status`, and the plan did not ask for filtering). **Closed:** the route resolves the credential to a developer and calls `registered_sessions(owner_id=...)`, which filters bindings by `owner_id` before computing their assignment state; `None` (the in-process default) still lists everyone, for the readers that are not a hook credential. A single-`WRITAI_HOOK_API_KEY` deployment registers every session under `default` and lists with that same key, so it sees everything it did before; `scripts/demo/demo_api.py`, `scripts/demo/check.sh` and `scripts/ci/writai_ci_check.py` all list with that single key. Tests (`test_supervisor_runtime.py`): `test_the_session_list_shows_each_developer_only_their_own` (alice and bob each start a session; each list holds exactly their own) and `test_a_single_shared_key_still_lists_every_session` (two sessions under one key, both listed, both `default`). `test_a_developer_cannot_end_another_developers_session` previously asserted that bob's list *showed* alice's session; it now asserts the list is empty, since that was the leak. Control: with the route hand-reverted to `registered_sessions()` the two-developer tests fail (`Left contains one more item: ('bob-session', 'bob')`) and the single-key test still passes; restored, all pass. | **RESOLVED.** Filtered by resolved developer; single-key unchanged. |
...
### Items carried into the final pass

- B2-1 (`.env.example` `WRITAI_HOOK_API_KEYS`) — **resolved** post-integration; see the B2 table.
- INT-2 (PR check grant validation), A1-1 to A1-5, DEMO-ENV-1 to DEMO-ENV-4 were
  carried into the final pass; the index at the top records their current status. The
  `outgoing_edges` divergence above, RC-1, RC-2 and B1-2 (orchestrator read-modify-write
  sites) are resolved, and so now are B1-3 (sibling stores named `.sqlite3`), B2-2
  (`HookApiKeyVerifier` alias), B2-4 (session list not filtered by owner), T0-1 (`uv.lock`,
  a stale entry: deleted in `7da68d2`), T0-2 (two unread env vars, dropped) and the
  `session_enforcement.py` transaction site disclosed under B1-1 (round 6). See each row.
