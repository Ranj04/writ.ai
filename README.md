...
The deterministic proof runs with no API keys and no database: `make demo` walks six stages end to end in about a tenth of a second. Behind it is one gate — `bash scripts/check.sh`, the same script CI runs and `make check` delegates to: the backend and frontend suites, ruff, mypy, a coverage floor with per-module sub-floors, typecheck and build. The badge above reports it on every push, which is a better source for the current counts than a number written here would be. Neo4j and Anthropic are optional extension points; the graph-store contract suite runs against a real Neo4j service container in CI.
