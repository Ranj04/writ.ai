"""Shared implementation for the Dragback Claude Code hooks.
...
1. **Privacy.** The PreToolUse request body carries exactly ``session_id``,
   ...
   once, because the service resolves the binding by reading ``.dragback/task``
...
DEFAULT_CACHE_PATH = ".dragback/hook-verdict-cache.json"

ENV_ENDPOINT = "DRAGBACK_HOOK_ENDPOINT"
ENV_TIMEOUT = "DRAGBACK_HOOK_TIMEOUT_SECONDS"
ENV_CACHE_PATH = "DRAGBACK_HOOK_CACHE_PATH"
ENV_API_KEY = "DRAGBACK_HOOK_API_KEY"
HOOK_API_KEY_HEADER = "X-Dragback-Hook-API-Key"
ENV_NOTIFY = "DRAGBACK_HOOK_NOTIFY"
...
TRUNCATION_NOTICE = "\n\n[dragback: context truncated to fit the 10000-character hook budget]"
...
NOTIFY_TITLE = "Dragback blocked a tool call"
...
REASON_UNREACHABLE_NO_CACHE = (
    "Dragback supervisor unreachable and no cached verdict for this session — failing closed."
)
REASON_INTERNAL_ERROR = (
    "The Dragback enforcement hook failed; failing closed rather than silently allowing."
)
REASON_NO_SESSION_ID = (
    "Dragback could not identify this session from the hook event — failing closed."
)
REASON_ALLOW_DEFAULT = "Dragback: session is authorized for its current assignment."
...
def http_transport(
    ...
    data = None
    headers = {"Accept": "application/json", "User-Agent": "dragback-hook/1"}
    if body is not None:
...
def build_additional_context(verdict: Dict[str, Any], stale: bool = False) -> str:
    ...
    lines: List[str] = [
        f"{_INDENT}{_LEADER_STOPPED}  DRAGBACK — the decision behind this task changed"
    ]
    if stale:
        lines += [
            "",
    ...
    if provenance or reason:
        ...
        lines.append(f"{_BODY}{''.ljust(5)}dragback dev why")
...
def decision_output(
    ...
    return {
        "hookSpecificOutput": {
            ...
            "permissionDecisionReason": one_line(reason) or "Dragback denied this tool call.",
...
def render_verdict(verdict: Dict[str, Any], stale: bool = False) -> Dict[str, Any]:
    ...
    if decision == ALLOW:
        ...
        if stale:
            context = (
                "Dragback supervisor unreachable; reusing the last cached verdict "
                "for this session, which was allow."
            )
            reason = reason or "Dragback: cached verdict for this session is allow."
        return decision_output(ALLOW, reason or REASON_ALLOW_DEFAULT, context)

    if decision != DENY:
        reason = f"Dragback returned an unrecognized verdict ({decision!r}) — failing closed."
    elif stale:
        cached_reason = one_line(verdict.get("reason"), 200)
        reason = (
            "Dragback supervisor unreachable; last cached verdict for this session denies. "
            + cached_reason
        )
    elif not reason:
        reason = "Dragback: this session's authorization was revoked."
...
def build_pre_tool_use_result(
    ...
    try:
        ...
        if not _is_well_formed(verdict):
            return deny_output(
                "Dragback supervisor returned an unusable response — failing closed."
            ), None
...
def read_marker_file(cwd: str, name: str) -> Optional[str]:
    """First non-empty line of ``.dragback/<name>``, or ``None``."""

    try:
        with open(str(Path(cwd) / ".dragback" / name), encoding="utf-8") as handle:
            for line in handle:
                stripped = line.strip()
                if stripped:
                    return stripped
...
def register_request_body(
    ...
    The service reads ``.dragback/attach`` and ``.dragback/task`` from the cwd
...
def describe_binding(response: Dict[str, Any]) -> str:
    ...
    if not isinstance(binding, dict):
        return "Dragback enforcement is active for this session."
    ...
    if not assignment_id:
        return (
            "Dragback enforcement is active. This session is registered but UNBOUND "
            "(no assignment matched), so tool calls are allowed and the session stays "
            "visible in `dragback dev status`."
        )
    return (
        f"Dragback enforcement is active. This session is bound to assignment "
...
def notify_denied(
    ...
    platform. Disable entirely with ``DRAGBACK_HOOK_NOTIFY=0``.
    """
