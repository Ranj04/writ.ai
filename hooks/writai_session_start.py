#!/usr/bin/env python3
"""writ.ai SessionStart hook — registers the session with the supervisor.

Reports facts only: cwd, git branch, ``.writai/attach``, ``.writai/task``. The
...
def build_output(
    ...
    try:
        ...
        if not session_id:
            return _context(
                "writ.ai could not register this session (no session id in the hook "
        ...
        try:
            ...
            return _context(
                "writ.ai supervisor unreachable at session start; this session is not "
                "registered. Start the agent service, or tool calls will be denied."
            )
        return _context(lib.describe_binding(response))
    except Exception:
        return _context("writ.ai session registration failed.")
...
def main(stdin: Optional[TextIO] = None, stdout: Optional[TextIO] = None) -> int:
    try:
        ...
        payload = _context("writ.ai session registration failed.")
