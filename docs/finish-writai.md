# Finish writ.ai — integration and completion
...
- the `.writai/attach` server-side wiring (Phase 2, item 3)
...
**2. `GET /supervisor/sessions`.** `supervisor_api.py` has `/check` and `/start` but no list route. `writai status` and `writai why` call it and are dead without it. Return assignment id, task id, binding state, snapshot and status per registered session.

---

## Phase 2 — Close every gap cut for time

**3. `.writai/attach` is not wired.** `cli_dev.py` writes the marker file; `ClaudeCodeSessionRegistry.attach()` is only ever called from tests. So `writai attach` is a no-op against the live service today — it writes a file the server never reads — and it is *first* in the documented binding resolution order. Wire it end to end. **Give this to sol.**
