...
4. **Agent, authority, and executor stay separate.** The planner cannot approve itself. `SupervisorRuntimeAdapter`'s own docstring says it: *"Agent-service runtime boundary; it never returns an authority verdict."* Keep that true.
...
- **Workspace document import** already implements the trust model: untrusted draft → OCR/extraction confidence in evidence metadata → explicit human confirmation of fields → recorded separately → then server validation. Slack is a new *source* for this path, not a new trust model.
...
**Hooks fail open.** On timeout, crash, or HTTP failure the tool call *proceeds*. Only an explicit `deny` blocks. Therefore the hook script must catch its own errors and emit `deny`, keep an explicit short timeout (the command default is 600s), and cache the last verdict on disk. Say this honestly — the PR check is the backstop.
...
**`CLAUDE.md` does not reload mid-session** — read at session start, explicitly excluded from hot-reload. Editing it is persistence for the *next* session, never an interrupt.
