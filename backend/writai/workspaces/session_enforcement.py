    ...
    def check(
        ...
        if binding is None:
            return ClaudeHookVerdict(
                ...
                reason=(
                    "writ.ai has no registered lifecycle record for this "
                    "Claude Code session."
                ),
        ...
        if binding.source is SessionBindingSource.UNRESOLVED_ATTACHMENT:
            ...
            # `.writai/attach` an off switch for enforcement, so it denies
            # until the marker is corrected or removed. Distinct from unbound,
            # which is genuinely "no binding information".
            return ClaudeHookVerdict(
                decision=HookPermissionDecision.DENY,
                reason=(
                    "writ.ai cannot resolve this session's explicit attachment: "
        ...
        if binding.assignment is None:
            return ClaudeHookVerdict(
                ...
                reason=(
                    "writ.ai session is registered but visibly unbound; "
        ...
        if assignment.state is SupervisorAssignmentState.CONTINUING:
            return ClaudeHookVerdict(
                decision=HookPermissionDecision.ALLOW,
                reason=(
                    "writ.ai preserved this assignment because its task scopes "
        ...
        return ClaudeHookVerdict(
            decision=HookPermissionDecision.ALLOW,
            reason="writ.ai assignment is bound to the current decision snapshot.",
            workspace_id=snapshot.locator.workspace_id,
            assignment_id=assignment.id,
            task_id=assignment.task_id,
            binding_source=binding.source,
...
def _deny_for_missing_assignment(
    ...
    return ClaudeHookVerdict(
        decision=HookPermissionDecision.DENY,
        reason=(
            "writ.ai session binding is stale because its assignment is no "
            "longer present."
        ),
