"""Developer-side writ.ai CLI group (Lane A).
...
here. This module must not import `writai.cli` at module scope: `cli.py`
...
if TYPE_CHECKING:  # pragma: no cover - typing only, no runtime cycle
    from writai.cli import WritaiClient
...
ENV_HOOK_API_KEY = "WRITAI_HOOK_API_KEY"
ATTACH_DIRECTORY = ".writai"
...
def register(
    ...
    """Register `writai dev ...`. Called once from `cli.build_parser`."""
...
def run(*, client: WritaiClient, args: argparse.Namespace) -> int:
    """Execute one `writai dev` command."""
...
def _run_attach(*, args: argparse.Namespace, json_output: bool) -> int:
    """Write `.writai/attach`. Local only — the service resolves the binding."""
...
def _run_status(
    *,
    ...
    for name, task, state, _source, bound in rows:
        if not bound:
            # A session writ.ai could not bind is allowed everything. It is
    ...
    *,
    ...
    if not _is_bound(binding):
        raise CliError(
            ...
            "assignment explains it. Run `writai dev attach` in its working "
            "directory.",
...
def _run_watch(
    *,
    ...
    from writai.cli import _redact
...
def _fetch_sessions(client: WritaiClient) -> tuple[JsonObject, list[JsonObject]]:
...
def _session_items(payload: Mapping[str, Any]) -> list[JsonObject]:
    ...
    raise CliError(
        "The writ.ai agent service did not return a session list.",
    ...
    if session_id is not None:
        for item in items:
            if _optional_text(_binding_of(item).get("session_id")) == session_id:
                return item
        raise CliError(
            f"No registered session {session_id}. Run `writai dev status`.",
