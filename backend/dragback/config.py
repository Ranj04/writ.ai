...
class Settings:
    ...
    authority_url: str = os.getenv(
        "DRAGBACK_AUTHORITY_URL", "http://localhost:8001"
    ).rstrip("/")
    agent_url: str = os.getenv("DRAGBACK_AGENT_URL", "http://localhost:8002").rstrip("/")
    executor_url: str = os.getenv(
        "DRAGBACK_EXECUTOR_URL", "http://localhost:8003"
    ).rstrip("/")
    service_timeout_seconds: float = float(os.getenv("DRAGBACK_SERVICE_TIMEOUT_SECONDS", "5"))
    execution_provider: str = os.getenv("DRAGBACK_EXECUTION_PROVIDER", "fixture").strip().lower()
    ...
    interrupt_escalation_threshold_seconds: float = float(
        os.getenv("DRAGBACK_INTERRUPT_ESCALATION_THRESHOLD_SECONDS", "300")
    )
    interrupt_escalation_scan_interval_seconds: float = float(
        os.getenv("DRAGBACK_INTERRUPT_ESCALATION_SCAN_INTERVAL_SECONDS", "30")
    )
    interrupt_escalation_grant_store: str = os.getenv(
        "DRAGBACK_INTERRUPT_ESCALATION_GRANT_STORE",
        ".dragback/interrupt-escalation-grants.json",
    )
    interrupt_escalation_phone_ref: str = os.getenv(
        "DRAGBACK_INTERRUPT_ESCALATION_PHONE_REF",
        "demo-venue",
    )
    ...
    dragback_hook_endpoint: str = os.getenv(
    ...
    dragback_hook_timeout_seconds: float = float(
        os.getenv("DRAGBACK_HOOK_TIMEOUT_SECONDS", "3")
    )
    dragback_hook_cache_path: str = os.getenv(
        "DRAGBACK_HOOK_CACHE_PATH",
        ".dragback/hook-verdict-cache.json",
    )
    public_base_url: str = os.getenv(
        "DRAGBACK_PUBLIC_BASE_URL",
        "http://localhost:8001",
    ).rstrip("/")
    approval_link_secret: str | None = (
        os.getenv("DRAGBACK_APPROVAL_LINK_SECRET") or None
    )
    approval_link_ttl_seconds: int = int(
        os.getenv("DRAGBACK_APPROVAL_LINK_TTL_SECONDS", "900")
    )
    approval_link_store: str = os.getenv(
        "DRAGBACK_APPROVAL_LINK_STORE",
        ".dragback/approval-link-uses.sqlite3",
    )
    approval_assertion_store: str = os.getenv(
        "DRAGBACK_APPROVAL_ASSERTION_STORE",
        ".dragback/approval-assertion-uses.sqlite3",
    )
    approval_recipient_bindings: str | None = (
        os.getenv("DRAGBACK_APPROVAL_RECIPIENT_BINDINGS") or None
    )
    ...
    email_from: str | None = os.getenv("DRAGBACK_EMAIL_FROM") or None
