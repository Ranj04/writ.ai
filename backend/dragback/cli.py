...
from dragback import cli_approve, cli_dev
...
class DragbackClient:
    ...
    def workspace_events(self, workspace_id: str) -> Iterator[JsonObject]:
        ...
        try:
            ...
            raise CliError(
                "The Dragback workspace event stream timed out.",
                code="TRANSPORT_TIMEOUT",
            ) from exc
        except httpx.RequestError as exc:
            raise CliError(
                f"Could not reach the Dragback workspace event stream: {exc}",
                code="TRANSPORT_ERROR",
            ) from exc
...
def parse_sse_lines(lines: Iterable[str]) -> Iterator[JsonObject]:
        ...
        if normalized == "":
            if data_lines:
                ...
                try:
                    ...
                    raise CliError(
                        "The Dragback event stream returned invalid JSON.",
                        code="INVALID_EVENT_STREAM",
                    ) from exc
                if not isinstance(payload, dict):
                    raise CliError(
                        "The Dragback event stream returned an invalid envelope.",
                        code="INVALID_EVENT_STREAM",
                    )
    ...
    if data_lines:
        try:
            ...
            raise CliError(
                "The Dragback event stream ended with invalid JSON.",
                code="INVALID_EVENT_STREAM",
            ) from exc
        if not isinstance(payload, dict):
            raise CliError(
                "The Dragback event stream ended with an invalid envelope.",
                code="INVALID_EVENT_STREAM",
            )
        yield {str(key): value for key, value in payload.items()}
...
def _safe_provider_args(values: Sequence[str]) -> tuple[str, ...]:
    ...
    for value in values:
        ...
        if option in _UNSAFE_PROVIDER_OPTIONS or unsafe_short_option:
            raise CliError(
                f"Provider argument {option!r} would override Dragback controls.",
...
def build_agent_prompt(
    ...
    *,
    ...
    return "\n".join(
        (
            "You are a coding subagent controlled by the Dragback supervisor.",
            ...
            "If Dragback interrupts this process, stop immediately.",
...
class AgentController:
    ...
    def preview_initial(self, workspace: Mapping[str, Any]) -> AgentAssignment:
        ...
        print("Command: " + shlex.join((*command[:-1], "<Dragback assignment prompt>")))
        print("Prompt:\n" + command[-1])
        return assignment
...
def build_parser() -> argparse.ArgumentParser:
    ...
    approve_change = command_parser(
        "approve-change",
        "Deprecated; use `dragback approve change`.",
    )
    ...
    agent = groups.add_parser(
        "agent",
        help="Run a provider CLI under Dragback supervisor control.",
    )
...
def _request_for_command(
    ...
    if command in {"approve-baseline", "approve-change"}:
        replacement = (
            ...
            else "`dragback approve change`"
        )
        raise CliError(
            f"The legacy `dragback workspace {command}` command is disabled; "
...
def _run_agent_command(
    *,
    ...
    try:
        ...
        controller.shutdown(reason="the Dragback wrapper stopped")
...
def run(
    ...
    *,
    ...
    try:
        ...
        print("dragback: interrupted; supervised process stopped.", file=sys.stderr)
        return 130
