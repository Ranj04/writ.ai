...
from dragback.workspaces.supervisor import SupervisorAssignment
...
class ClaudeCodeSessionRegistry:
    ...
    explicit attachment, task ID in the branch, ``.dragback/task``, unbound.
    """
...
def _resolve_binding(
    *,
    ...
    if attachment is not None:
        ...
        if len(matches) == 1:
            return _bound(
                ...
                target=matches[0],
                detail="Bound by an explicit dragback attach selection.",
            )
    ...
    # `dragback dev why` says the marker was skipped and why. This also matches
    # `scripts/ci/dragback_ci_check.py`, whose `read_marker_file` returns None
    ...
    if file_attachment.present and file_attachment.assignment_id is not None:
        ...
        if len(matches) == 1:
            return _bound(
                ...
                source=SessionBindingSource.EXPLICIT,
                target=matches[0],
                detail="Bound by the repository .dragback/attach file.",
            )
        if len(matches) > 1:
            return _unresolved_attachment(
                session_id=session_id,
                ...
                branch=branch,
                detail=(
                    ".dragback/attach matches assignments in multiple "
                    "workspaces."
                ),
            )
        return _unresolved_attachment(
            ...
            branch=branch,
            detail=(
                ".dragback/attach does not name an available live "
                "assignment."
            ),
        )
...
def _with_skipped_marker(detail: str, reason: str) -> str:
    ...
    combined = f"{detail} (.dragback/attach was ignored: {reason})"
...
def _resolve_by_branch_then_task_file(
    *,
    ...
    """The rules after the explicit slot: branch task ID, then ``.dragback/task``."""
    ...
    if task_id is not None:
        ...
        if len(task_matches) == 1:
            return _bound(
                ...
                detail="Bound by the repository .dragback/task file.",
            )
        if len(task_matches) > 1:
            return _unbound(
                session_id=session_id,
                cwd=cwd,
                branch=branch,
                detail=".dragback/task matches assignments in multiple workspaces.",
            )
        return _unbound(
            session_id=session_id,
            cwd=cwd,
            branch=branch,
            detail=".dragback/task does not name an available live assignment.",
        )
    ...
    return _unbound(
        ...
        branch=branch,
        detail=(
            "No explicit attachment, branch task ID, or .dragback/task binding "
...
def _read_attach_file(cwd: str) -> _AttachFileResult:
    ...
    try:
        ...
        return _AttachFileResult(
            present=True,
            detail=".dragback/attach could not be read safely.",
        )
    if stat.S_ISLNK(metadata.st_mode):
        return _AttachFileResult(
            present=True,
            detail=".dragback/attach must not be a symbolic link.",
        )
    if not stat.S_ISREG(metadata.st_mode):
        return _AttachFileResult(
            present=True,
            detail=".dragback/attach must be a regular file.",
        )
    if metadata.st_size > _MAX_TASK_FILE_BYTES:
        return _AttachFileResult(
            present=True,
            detail=".dragback/attach exceeds the safe size limit.",
        )
    try:
        content = path.read_text(encoding="utf-8")
    except (OSError, UnicodeError):
    ...
    if len(lines) != 1:
        return _AttachFileResult(
            present=True,
            detail=".dragback/attach must contain exactly one assignment ID.",
        )
    if len(lines[0]) > 255:
        return _AttachFileResult(
            present=True,
            detail=".dragback/attach contains an invalid assignment ID.",
        )
