...
from dragback.llm.extractor import evidence_span_error
...
def register(
    ...
    """Register `dragback approve ...`. Called once from `cli.build_parser`."""
...
def _select_intake_workspace(
    ...
    selected_id = (
        ...
        or os.getenv("DRAGBACK_WORKSPACE_ID", "").strip()
    )
    if selected_id:
...
def _propose_text_decision(
    *,
    ...
    if not raw_text:
        raise CliError(
            "dragback approve requires --text, or a pending/change subcommand.",
...
def run(*, client: ApprovalClient, args: argparse.Namespace) -> int:
    ...
    if args.command is None:
        ...
        if (
            ...
            print(
                "dragback: the requested Decision is not awaiting approval. "
                "[PENDING_DECISION_NOT_FOUND]",
                file=sys.stderr,
            )
            return 2
        pending = workspace_pending
    approval_token = os.getenv("DRAGBACK_HOOK_API_KEY", "").strip()
    if not approval_token:
        print(
            "dragback: DRAGBACK_HOOK_API_KEY is required for authenticated "
