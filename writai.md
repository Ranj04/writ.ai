...
**writ.ai continuously verifies that an autonomous coding agent is still working toward the company's latest approved intent—and forces it to replan when the decisions behind its task change.**
...
writ.ai addresses that gap. It does not merely ask whether an agent has permission to act. It asks whether the objective driving the action is still authorized by current company decisions.
...
6. writ.ai follows the provenance path from the decision through the specification and ticket to the active plan.
...
writ.ai models company artifacts as typed nodes:
...
The newest message does not automatically win. writ.ai evaluates:
...
writ.ai makes this dependency chain explicit and auditable. The changed decision does not need to mention the downstream ticket directly.
...
Each descendant artifact also has scopes. writ.ai invalidates only descendants whose scopes intersect the changed decision's scope.
...
When the graph changes to `graph-v18`, writ.ai reevaluates affected active runs. A previously valid authorization can become unusable without anyone editing the original ticket.
...
writ.ai does not stop at a warning banner. The mock PR gateway verifies the authorization before applying an action:
...
LangGraph is preferred over CrewAI because writ.ai is primarily an explicit, deterministic state machine around one coding agent—not a crew of role-based agents delegating work to one another.
...
- writ.ai focuses on **mid-run reauthorization when the upstream decision that justified the work changes**.
...
1. **Problem — 20–30 seconds**  
   “Coding agents can write correct code from obsolete decisions. writ.ai verifies that the work is still wanted.”
